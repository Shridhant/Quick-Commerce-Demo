...
export const saveUploadedProductFile = async (
  file: UploadedFile,
  rootFolder: string,
  id : string,
): Promise<string> => {
  const cleanId = uuidv4().replace(/-/g, ""); 
  const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
  const fileExt = path.extname(file.originalname);
  const fileName = `${id}-${cleanId}-${uniqueSuffix}${fileExt}`;
  const uploadFolder = `${rootFolder}/ProductImages`;
  const filePath = path.join(uploadFolder, fileName);
